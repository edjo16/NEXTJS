...
import { useContactIndividual } from "../../hooks/useContactIndividual";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

interface ContactFormIndividualProps {
  userRef?: { name: string; email?: string; [key: string]: any };
}

export default function ContacFormIndividual({ userRef }: ContactFormIndividualProps) {
  //@ts-ignore
  ...
  } = useContactIndividual();

  // Para checks verdes
  const watchedFields = watch();

  const handleFormSubmit = async (data: any) => {
    // Attach userRef to the data
    const submitData = {
      ...data,
      userRef: userRef ? { name: userRef.name, email: userRef.email } : undefined,
    };
    ...
    <>
      ...
      <section className="mb-16">
        ...
        <h2 className="text-2xl font-bold text-primary-500 mb-6">
          Get in touch with {userRef?.name ? userRef.name : "us"}
        </h2>
        <form
