...
// tu array countries completo aquí
...
>(
  (
    ...
    const handlePhoneChange = (
      ...
      updateValue(selectedCountryCode, phone);
    };
    ...
    return (
      <div className="relative">
        {label && (
          <label
            htmlFor={name}
            className="absolute -top-2 left-3 z-10 bg-white px-1 text-xs text-gray-500"
          >
            {label}
          </label>
        )}
        ...
        >
          ...
          <input
            ...
            className="flex-1 bg-white px-3 focus:outline-none"
          />
        </div>

        {isValid && (
          <CircleCheck className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-primary-500" />
        )}

        {error && (
          <CircleAlert className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-red-500" />
        )}
      </div>
    );
  }
...
FloatingLabelPhoneInput.displayName =
