...
export function FloatingLabelPhoneInput({
  ...
  const sanitizePhoneNumber = (raw: string) => raw.replace(/[\s-]/g, "")
  ...
  const handleCountryChange = (countryCode: string) => {
    ...
    if (country) {
      ...
      const fullNumber = cleanNumber ? `${country.dialCode} ${cleanNumber}` : country.dialCode
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const cleanNumber = sanitizePhoneNumber(newPhoneNumber)
  ...
  const isValid = !error && value !== undefined && value !== "" && phoneNumber !== ""
      ...
      <div className="relative">
        <label
          ...
          className={cn(
            ...
            showError ? "text-red-500" : isValid ? "text-green-600" : "text-gray-500",
          )}
        >
          {label}
        </label>
        <FormControl>
          <div
            className={cn(
              "flex h-14 w-full rounded-md border focus-within:outline-none focus-within:ring-1 focus-within:ring-offset-0",
              showError
          ...
          >
              ...
              <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    role="combobox"
                    aria-expanded={open}
                    className="h-full w-auto min-w-[100px] justify-between"
                  >
                    <div className="flex items-center gap-1">
                      <span className="text-sm">{selectedCountry.flag}</span>
                      <span className="text-sm font-regular">{selectedCountry.dialCode}</span>
                    </div>
            ...
            <input
              ...
              className="flex-1 px-2 py-3 text-sm bg-white focus:outline-none rounded-r-md"
            />
          </div>
        </FormControl>
