...
export async function fetchInitialData(): Promise<HomePageData> {
  ...
  const response2 = await fetch(`${apiUrl}/items/news?fields=id,annual_report_link,is_annual_report,code,title,preview_image,tags,title_preview,content_preview,date,index_articles_home&filter[display_at_home][_eq]=true`, {
